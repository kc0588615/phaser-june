...
7. Player matches gems → objective progress → `node-objective-updated` → auto-complete at target
8. `node-complete` → MainAppLayout advances to next node (or completes run)
...
- `nodeRequiredGems: Set<string>` — colors that count toward objective
...
Progress emitted via `node-objective-updated` event. ActiveEncounterPanel shows progress bar.
...
| `src/game/scenes/Game.ts` | Objective tracking, encounter triggers, applyEncounter |
| `src/MainAppLayout.tsx` | Run phase state machine, event wiring, node advancement |
| `src/components/ActiveEncounterPanel.tsx` | Node info panel, objective progress bar, encounter flash |
