...
A Next.js + React application that embeds a Phaser 3 puzzle game and a Cesium 3D map, with species and habitat data stored in Postgres via Prisma ORM. Server runtime is required for API routes and Prisma. Auth is planned via Clerk (not implemented yet). UI built with Tailwind CSS and shadcn/ui.

## Tech Stack

- Phaser 3.90.0 for gameplay
- Next.js 16.1.0 with server runtime (API routes + Prisma)
- TypeScript 5
- Cesium + Resium for 3D map
- Postgres (Hetzner VPS) + Prisma ORM for data
...
- `src/lib/` — Clients and services: `prisma.ts`, `speciesQueries.ts`
...
- Prisma data layer in `src/lib/speciesQueries.ts`
...
Note: This project requires the Next.js server runtime for API routes and Prisma. Static export is disabled in `next.config.mjs`.
...
- Data: [`DATABASE_USER_GUIDE.md`](./DATABASE_USER_GUIDE.md), [`SPECIES_DATABASE_IMPLEMENTATION.md`](./SPECIES_DATABASE_IMPLEMENTATION.md), [`PRISMA_ORM_GUIDE.md`](./PRISMA_ORM_GUIDE.md)
