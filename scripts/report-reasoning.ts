...
type ReasoningEvent = { obsRef?: string; correct?: boolean; latencyMs?: number };
type Issuance = { ref?: string; cardId?: number };
type RunRow = { id: string; metadata: Record<string, unknown> };
type NodeRow = { run_id: string; objective_target: number; objective_progress: number; moves_used: number };
  ...
  try {
    const runs = await sql<RunRow[]>`
      ...
      WHERE metadata ? 'casePublic'
        AND jsonb_array_length(COALESCE(metadata->'reasoningEvents', '[]'::jsonb)) > 0
      ORDER BY started_at DESC
      LIMIT 500
    `;
    const nodes = runs.length ? await sql<NodeRow[]>`
      SELECT run_id::text, objective_target, objective_progress, moves_used
      FROM eco_run_nodes
    ...
    console.log(`Runs with interpretations: ${runs.length}`);
