    ...
    >
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          ...
          {hasValue(species.iucn_url) && (
            <Button asChild variant="ghost" size="sm" className="text-slate-300 hover:text-white text-xs">
              <Link href={species.iucn_url!} target="_blank" rel="noopener noreferrer" aria-label="Open IUCN page in a new tab">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm w-full">
          {[
            [
              ...
              { label: 'Order', value: species.taxon_order },
            ],
            [
      ...
      {/* Conservation Status */}
      {(hasValue(species.conservation_text) || hasValue(species.threats)) && (
        <>
          <div className="h-px bg-border my-4 sm:my-6" />
          <div className="mb-4 sm:mb-6">
            ...
            <div className="bg-orange-400/15 border border-orange-400/40 rounded-lg p-4 sm:p-5">
              {hasValue(species.conservation_text) && (
                <p
                ...
                >
                  <strong className="text-orange-200">Conservation Notes:</strong> {species.conservation_text}
                </p>
              )}
      ...
      {/* Habitat Section */}
      {(hasValue(species.habitat_tags) || hasValue(species.habitat_description) || species.marine || species.terrestrial || species.freshwater) && (
        <>
          <div className="h-px bg-border my-4 sm:my-6" />
          <div className="mb-4 sm:mb-6">
            ...
            <div>
              ...
              {hasValue(species.habitat_description) && (
                <p 
      ...
      {hasValue(species.geographic_description) && (
        <>
          <div className="h-px bg-border my-4 sm:my-6" />
          <div className="mb-4 sm:mb-6">
      ...
      {(hasValue(species.bioregion) || hasValue(species.realm) || hasValue(species.subrealm) || hasValue(species.biome)) && (
        <>
          <div className="h-px bg-border my-4 sm:my-6" />
          <div className="mb-4 sm:mb-6">
            ...
            <div className="grid grid-cols-[minmax(0,auto)_1fr] gap-x-3 sm:gap-x-4 gap-y-2 text-sm sm:text-base items-start">
              {hasValue(species.bioregion) && (
                <>
                  <span className="text-slate-300 font-medium">Bioregion:</span>
                  <span className="text-white grid-value" style={{ wordBreak: 'normal', overflowWrap: 'normal' }}>{species.bioregion}</span>
                </>
              )}
              ...
              {hasValue(species.subrealm) && (
                <>
                  <span className="text-slate-300 font-medium">Sub-realm:</span>
                  <span className="text-white grid-value" style={{ wordBreak: 'normal', overflowWrap: 'normal' }}>{species.subrealm}</span>
                </>
              )}
      ...
      {(hasValue(species.color_primary) || hasValue(species.color_secondary) || hasValue(species.pattern) || hasValue(species.size_min_cm) || hasValue(species.size_max_cm) || hasValue(species.weight_kg) || hasValue(species.shape_description)) && (
        <>
          <div className="h-px bg-border my-4 sm:my-6" />
          <div className="mb-4 sm:mb-6">
            {getCategoryHeader('🐾', 'orange', 'Physical Characteristics')}
            <div className="grid grid-cols-[minmax(0,auto)_1fr] gap-x-3 sm:gap-x-4 gap-y-2 text-sm sm:text-base items-start">
              {hasValue(species.color_primary) && (
                <>
                  <span className="text-slate-300 font-medium">Primary Color:</span>
                  <span className="text-white grid-value" style={{ wordBreak: 'keep-all', overflowWrap: 'break-word' }}>{species.color_primary}</span>
                </>
              )}
              {hasValue(species.color_secondary) && (
                <>
                  <span className="text-slate-300 font-medium">Secondary Color:</span>
                  <span className="text-white grid-value" style={{ wordBreak: 'keep-all', overflowWrap: 'break-word' }}>{species.color_secondary}</span>
                </>
              )}
              ...
              {(hasValue(species.size_min_cm) || hasValue(species.size_max_cm)) && (
                <>
                  ...
                  <span className="text-white grid-value" style={{ wordBreak: 'keep-all', overflowWrap: 'break-word' }}>
                    {hasValue(species.size_min_cm) && hasValue(species.size_max_cm) 
                      ? `${species.size_min_cm} - ${species.size_max_cm} cm`
                      : hasValue(species.size_min_cm) ? `${species.size_min_cm} cm` : `${species.size_max_cm} cm`}
                  </span>
                </>
              )}
              ...
              {hasValue(species.shape_description) && (
                <>
                  <span className="text-muted-foreground">Shape:</span>
                  <span className="text-white grid-value" style={{ wordBreak: 'keep-all', overflowWrap: 'break-word' }}>{species.shape_description}</span>
                </>
              )}
            </div>
          </div>
