...
export interface CompiledCaseV3 {
  version: 3;
  public: { version: 3; candidateIds: number[]; boardSeeds: [number, number, number]; mapView: ExpeditionMapView };
  private: {
    version: 3;
    answerId: number;
    caseSeed: string;
...
export type CompileCaseV3Result = CompiledCaseV3 | { error: string; message: string };

export interface CompileCaseV3Input {
  caseSeed: string;
...
export function compileCaseV3(input: CompileCaseV3Input): CompileCaseV3Result {
  ...
  const profileById = new Map(input.speciesPool.filter(profile => ids.includes(profile.speciesId)).map(profile => [profile.speciesId, profile]));
  ...
  for (const speciesId of ids) {
    ...
    if (!cards || cards.length !== EVIDENCE_FAMILIES.length) return fail('invalid_cards', 'Every species needs five family cards.');
    ...
    for (const card of cards) {
      ...
      const frequency = [...profileById.values()].filter(candidate =>
    ...
    cardIdsBySpecies.set(speciesId, Object.fromEntries(EVIDENCE_FAMILIES.map(family => [family, byFamily.get(family)!.id])) as Record<EvidenceFamily, number>);
    ...
    for (const family of EVIDENCE_FAMILIES) {
      ...
      if (familyHints.length < 3 || familyHints.length > 5
        || familyHints.some((hint, index) => !Number.isSafeInteger(hint.id) || hint.id <= 0
          ...
          || !hintWeakensCard(card, hint, [...profileById.values()]))) {
  ...
  return {
    version: 3,
    public: { version: 3, candidateIds: shuffle(ids, createSeededStream(input.caseSeed, 'candidate-shuffle-v3')), boardSeeds: [...input.boardSeeds] as [number, number, number], mapView },
    private: {
      version: 3,
      answerId,
      caseSeed: input.caseSeed,
      familyCardIds: cardIdsBySpecies.get(answerId)!,
      familyHintIds: hintIdsBySpecies.get(answerId)!,
