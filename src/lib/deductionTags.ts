export const TAG_VOCAB = {
  ...
  research_signal: [
    ...
    'larger_than_mouse',
    'subtonne_body_mass',
    'non_scaled_body_cover',
    'not_large_prey_specialist',
    'ground_contact_travel',
    'large_ground_impression',
    'digging_trace',
    'tree_feeding_debris',
    'herd_group',
    'vertebrate_predation',
    'arboreal_foraging',
    'contact_calls',
    'substrate_vibration',
    'solitary_quiet',
    ...
    'keratin_defense',
    'nonvisual_foraging',
    'endangered_megafauna_cohort',
  ],
  gameplay_size: ['large', 'small_medium'],
  food_source: ['plants', 'animals'],
  social_pattern: ['group', 'solitary'],
  superorder: ['laurasiatheria', 'afrotheria'],
  comparison_lineage: ['not_bovid', 'not_golden_mole', 'not_elephant', 'not_pangolin', 'not_big_cat', 'not_fruit_bat'],
  comparison_form: ['not_horned_antelope', 'not_sand_digger', 'not_trunked_giant', 'not_scaled_digger', 'not_striped_cat', 'not_winged_mammal'],
  comparison_behavior: ['not_desert_herd', 'not_sand_swimmer', 'not_matriarchal_herd', 'not_defensive_curler', 'not_solitary_ambusher', 'not_canopy_colony'],
  comparison_diet: ['not_desert_browser', 'not_soil_insect_hunter', 'not_bulk_browser', 'not_ant_specialist', 'not_large_prey_hunter', 'not_fruit_flower_forager'],
  comparison_range: ['not_sahara_sahel', 'not_namaqualand_dunes', 'not_south_asian_mainland', 'not_southeast_asian_forest', 'not_asian_big_cat_range', 'not_comoros_islands'],
} as const;
...
const VOCAB_KEYS_BY_PROFILE_CATEGORY: Record<DeductionProfileCategory, readonly TagCategory[]> = {
  habitat: ['habitat_tag'],
  morphology: ['size_bucket', 'body_plan', 'distinctive_features', 'gameplay_size', 'comparison_form'],
  diet: ['diet_type', 'food_source', 'comparison_diet'],
  behavior: ['activity_pattern', 'sociality', 'locomotion', 'foraging_style', 'social_pattern', 'comparison_behavior'],
  reproduction: ['parity', 'care_pattern', 'lifespan_bucket'],
  taxonomy: ['superorder', 'comparison_lineage'],
  geography: ['climate_zone', 'comparison_range'],
  conservation: ['threat_tag'],
  key_fact: ['research_signal'],
};
